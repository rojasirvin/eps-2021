...
$$V(\hat{\beta}_{het.})=\left(\begin{matrix} \hat{\sigma}_{1} & 0  & 0  & \ldots & 0 \\ 0 & \hat{\sigma}_{2} & 0 & \ldots & 0 \\ \vdots & & & & \\ 0 & & &  \ldots & \hat{\sigma}_{N}\end{matrix}\right)$$
---

# Errores estándar agrupados

- Para estimar la varianza empleamos una generalización de la propuesta de White para errores robustos

- Si $G\to\infty$, el estimador de la matriz de errores agrupados robusta (CRVE) es consistente para estimar $V(\hat{\beta})$:

$$\hat{V}_{CRVE}(\hat{\beta})=(X'X)^{-1}\left(\sum_{g=1}^G X_g'\hat{u}_g\hat{u}_g'X_g\right)(X'X)^{-1}$$
...
- En cambio, con errores agrupados, la matriz de varianzas tiene una estructura por bloques:

$$\small V(\hat{\beta}_{agrup.})=\left(\begin{matrix} \hat{\sigma}_{1,1}^2 & \hat{\sigma}_{12,1} & \ldots & \hat{\sigma}_{1n,1} & 0 & 0 & \ldots &  0 & \ldots & 0 & 0 & \ldots &  0 \\ \hat{\sigma}_{21,1} & \hat{\sigma}_{2,1}^2 & \ldots & \hat{\sigma}_{2n,1} & 0 & 0 & \ldots & 0 & \ldots  & 0 & 0 & \ldots &  0\\ 
\vdots & \vdots  & & \vdots & \vdots & \vdots  & &  \vdots& & \vdots & \vdots &  &  \vdots \\ \hat{\sigma}_{n,1} & \hat{\sigma}_{n,2}& \ldots & \hat{\sigma}_{n,1}^2& 0 & 0 &\ldots & 0 & \ldots & 0 & 0 & \ldots &  0 \\  0 & 0 & \ldots &  0 & \hat{\sigma}_{1,2}^2 & \hat{\sigma}_{12,2} & \ldots & \hat{\sigma}_{1n,2} &\ldots & 0 & 0 & \ldots &  0  \\ 0 & 0 & \ldots &  0 & \hat{\sigma}_{21,2} & \hat{\sigma}_{2,2}^2 & \ldots & \hat{\sigma}_{2n,2} &\ldots & 0 & 0 & \ldots &  0 \\ \vdots & \vdots  & & \vdots & \vdots & \vdots  & &  \vdots& & \vdots & \vdots &  &  \vdots  \\ 0 & 0 & \ldots &  0 & \hat{\sigma}_{n1,2} & \hat{\sigma}_{n2,2} & \ldots & \hat{\sigma}_{n,2}^2 &\ldots & 0 & 0 & \ldots &  0 \\ \vdots & \vdots  & & \vdots & \vdots & \vdots  & &  \vdots& & \vdots & \vdots &  &  \vdots \\ 0 & 0 & \ldots &  0 & 0 &  0 & \ldots & 0 &\ldots & \hat{\sigma}_{1,G}^2 & \hat{\sigma}_{12,G} & \ldots &  \hat{\sigma}_{1n,G} \\  0 & 0 & \ldots &  0 & 0 &  0 & \ldots & 0 &\ldots & \hat{\sigma}_{21,G} & \hat{\sigma}_{2,G}^2 & \ldots &  \hat{\sigma}_{2n,G} \\ \vdots & \vdots  & & \vdots & \vdots & \vdots  & &  \vdots& & \vdots & \vdots &  &  \vdots \\  0 & 0 & \ldots &  0 & 0 &  0 & \ldots & 0 &\ldots & \hat{\sigma}_{n1,G} & \hat{\sigma}_{n2,G} & \ldots &  \hat{\sigma}_{n,G}^2 \end{matrix}\right)$$
